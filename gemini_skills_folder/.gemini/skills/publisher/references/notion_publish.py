    ...
    try:
        if not sys.stdin.isatty():
            ...
            if raw_input.strip():
                ...
                _publish_to_notion_rest(data)
                print("Success!")
        else:
            try:
                with open("notion_payload.json", "r", encoding="utf-8") as f:
                    data = json.load(f)
                _publish_to_notion_rest(data)
                print("Success!")
            except FileNotFoundError:
                print("Error: notion_payload.json not found and no data piped to stdin.")
            except json.JSONDecodeError:
                print("Error: Could not decode JSON from notion_payload.json.")
    except Exception as e:
