...
Scans .claude/skills/*/SKILL.md to discover skills, then either:
  - Loads a custom Python class from references/<name>.py (if it exists)
...
class SkillRegistry:
    """Discovers and loads skills from .claude/skills/*/SKILL.md."""
    ...
    def discover(self) -> list[dict]:
        ...
        for skill_md in sorted(self.skills_dir.glob("*/SKILL.md")):
            ...
            py_path = skill_dir / "references" / f"{name}.py"
            has_custom = py_path.exists()

            self._skill_metadata.append({
                "name": name,
                "description": description,
                "fallback_prompt": fallback_prompt,
                "has_custom_class": has_custom,
                "class_path": py_path if has_custom else None,
                "skill_dir": skill_dir,
            })
    ...
    def load_all(
        ...
        Args:
            ...
            extra_kwargs: Optional per-skill extra constructor kwargs,
                e.g. {"publisher": {"notion_mcp_client": mcp_client}}
        """
