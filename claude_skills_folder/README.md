...
A proof-of-concept for building reusable AI workflows in Claude Code using **Skills** and **Commands**.

---

## What Are Skills?

A **Skill** is a reusable, role-specific prompt module stored under `.claude/skills/<skill-name>/SKILL.md`. Each skill defines a persona, a behavior, and a structured output format.

Skills are the *logic layer* — they contain the detailed instructions for how Claude should think, decompose a problem, and format its response. Skills do not run on their own; they are invoked by commands or referenced directly in conversation.
...
A **Command** is a short prompt file stored under `.claude/commands/<command-name>.md`. It acts as a *thin entry point* that invokes a skill and passes user-supplied arguments via `$ARGUMENTS`.

Commands are registered as slash commands inside Claude Code, making them directly executable from the CLI:
...
### 2. Writer
**File:** `.claude/skills/writer/SKILL.md`
...
### 3. Reviewer
**File:** `.claude/skills/reviewer/SKILL.md`
...
### 4. Publisher
**File:** `.claude/skills/publisher/SKILL.md`
...
| **Discoverability** | Listed in Claude Code as registered commands | Hidden; must be remembered |
...
claude -p "/pipeline Write about AI trends and publish to Notion"
```
